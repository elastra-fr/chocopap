  ...
  return (
    ...
    >
      <div className="carouselWrapper">
        ...
        <div className="carouselDots">
          {images.map((_, index) => {
            return (
              <div
                key={index}
                className={index == current ? "imgDot imgDotActive" : "imgDot"}
                onClick={() => {
                  setCurrent(index);
                }}
              ></div>
        ...
        <div className={`mainTitle themeTitle${current}`}>
          <h1>CHOCO PAP</h1>
        </div>
        <Link className={`mainButton themeMainButton${current}`} to="/Boutique">
          VOIR LA BOUTIQUE
        </Link>
      </div>
    </div>
